...
@test "use Go version 1.12.7 as default" {
    run /var/lib/tsuru/deploy
    [ "$status" -eq 0 ]
    [[ "$output" == *"Using Go version: go version go1.12.7 linux/amd64"* ]]
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"go version go1.12.7 linux/amd64"* ]]
}
...
@test "use existing Go version from GO_VERSION" {
    ...
    [[ "$output" == *"Using Go version: go version go1.11.12 linux/amd64"* ]]
